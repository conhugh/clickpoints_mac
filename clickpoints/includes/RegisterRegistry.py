...
def install(mode="install", extension=None):
    ...
    if mode == 'install':
        ...
        if sys.platform.startswith('win'):
            ...
            application_path = "/Users/" + os.popen('whoami').read()[:-1] + "/.local/share/applications/"
            if not os.path.exists(application_path):
                os.makedirs(application_path)

            desktop_file = "/Users/" + os.popen('whoami').read()[:-1] + "/.local/share/applications/clickpoints.desktop"
            ...
            for ext in ["application/cdb", "video/mp4", "video/x-msvideo", "video/mpeg", "image/bmp", "image/png",
